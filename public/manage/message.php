						...
						<?php
							...
							if(!$flag){
								...
								if($num_rows > 1){
									...
									$result = $con->select('tb_test','','','','',"$limit",'10');
								}
							}
							
							foreach($result as $key=>$row){
								echo "<tr>";
								echo "<td width='160' align='left'>".$row['message_id']."</td>";
								echo "<td width='360' align='left'>"."发布文章"."</td>";
								echo "<td width='560' align='left'>".date('Y-m-d H:i:s',$row['message_publish'])."</td>";
								echo "<td width='180' align='left'>"."发布用户"."</td>";
								if($row['message_content'] == ''){
									echo "<td width='560' align='left'>".$row['message_content']."</td>";
								}else{
									echo "<td width='560' align='left'>".mb_substr($row['message_content'],0,10,'utf8').'...'."</td>";
								}
								echo "<td width='160' align='left'><a class=\"del\" href='javascript:void(0);'>删除<span style = \"display:none\">".$row['message_id']."</span></a></td>";
								echo "</tr>";
							}
							//$flag = true;
						?>
			...
			$(document).ready(function(){
				...
				for (var i = 0; i < del.length; i++) {
					$(del[i]).click(function(){
						...
						$.post("del.php", {pid:val,state:'5'}, function(data){
							if(data == '1'){
