				...
				<table border="1" width="980" height="60" cellspacing="0" cellpadding="6">
					...
					<tbody>
						<?php
							...
							if(!$flag){
								$con = new connect();
								$num_rows = $con->num_rows('tb_teacher');
								if($num_rows > 1){
									...
									$result = $con->select('tb_teacher','','','','',"$limit",'10');
								}
							}
							
							foreach($result as $key=>$row){
								echo "<tr>";
								echo "<td width='160' align='left'>".$row['teacher_id']."</td>";
								...
								echo "<td width='560' align='left'>".$row['account']."</td>";
								echo "<td width='160' align='left'><a class=\"del\" href='javascript:void(0);'>删除<span style = \"display:none\">".$row['teacher_id']."</span></a></td>";
								echo "</tr>";
			...
			$(document).ready(function(){
				...
				for (var i = 0; i < del.length; i++) {
					$(del[i]).click(function(){
						...
						$.post("del.php", {pid:val,state:'3'}, function(data){
							if(data == '1'){
