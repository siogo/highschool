						...
						<?php
							...
							if(!$flag){
								...
								if($num_rows > 10){
									...
									$result = $con->select('tb_onlinestu','','','','',"$limit",'10');
								}
							}
							
							foreach($result as $key=>$row){
								echo "<tr>";
								echo "<td width='160' align='left'>".$row['online_id']."</td>";
								echo "<td width='360' align='left'>".mb_substr($row['online_title'],0,10,'utf-8')."</td>";
								echo "<td width='560' align='left'>".date('Y-m-d H:i:s',$row['online_publishtime'])."</td>";
								echo "<td width='160' align='left'>".$row['online_author']."</td>";
								// if($row['para_content'] == ''){
